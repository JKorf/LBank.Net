# ![LBank.Net](https://raw.githubusercontent.com/JKorf/LBank.Net/main/LBank.Net/Icon/icon.png) LBank.Net  

[![.NET](https://img.shields.io/github/actions/workflow/status/JKorf/LBank.Net/dotnet.yml?style=for-the-badge)](https://github.com/JKorf/LBank.Net/actions/workflows/dotnet.yml) ![License](https://img.shields.io/github/license/JKorf/LBank.Net?style=for-the-badge)

[![Docs](https://img.shields.io/badge/Docs-LBank.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=LBank.Net)

LBank.Net is a client library for accessing the [LBank REST and Websocket API](https://www.lbank.com/docs/#introduction). 
...
*Note the order API is currently not publicly accessible*

## Documentation

The [LBank.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=LBank.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=LBank.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=LBank.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=LBank.Net) | Client interfaces, methods, and properties |
...
### NuGet 
[![NuGet version](https://img.shields.io/nuget/v/LBank.net.svg?style=for-the-badge)](https://www.nuget.org/packages/LBank.Net)  [![Nuget downloads](https://img.shields.io/nuget/dt/LBank.Net.svg?style=for-the-badge)](https://www.nuget.org/packages/LBank.Net)

	dotnet add package LBank.Net
	
### GitHub packages
LBank.Net is available on [GitHub packages](https://github.com/JKorf/LBank.Net/pkgs/nuget/LBank.Net). You'll need to add `https://nuget.pkg.github.com/JKorf/index.json` as a NuGet package source.

### Download release
[![GitHub Release](https://img.shields.io/github/v/release/JKorf/LBank.Net?style=for-the-badge&label=GitHub)](https://github.com/JKorf/LBank.Net/releases)

The NuGet package files are added along side the source with the latest GitHub release which can found [here](https://github.com/JKorf/LBank.Net/releases).
...
var lastPrice = tickerResult.Data.Single().Ticker.LastPrice;
```

*Place order:*
```csharp
var restClient = new LBankRestClient(opts => {
	opts.ApiCredentials = new LBankCredentials("APIKEY", "APISECRET");
});

// Place Limit buy order for 0.1 ETH at 2000
...
For more examples and explanations, continue with the [LBank.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=LBank.Net) or browse the [compilable repository examples](https://github.com/JKorf/LBank.Net/tree/main/Examples).

## AI / LLM documentation

LBank.Net includes AI-oriented documentation and examples for code generation tools:

|File|Purpose|
|--|--|
|[`AGENTS.md`](AGENTS.md)|Assistant skill with core LBank.Net patterns, pitfalls, and examples|
...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `ILBankSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new LBankRestClient();
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `ILBankSharedApiClient` to access all of the exchange's Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.

## CryptoExchange.Net
LBank.Net is based on the [CryptoExchange.Net](https://github.com/JKorf/CryptoExchange.Net) base library. Other exchange API implementations based on the CryptoExchange.Net base library are available and follow the same logic.
...
|--|--:|--|
|Common Endpoints|✓|`restClient.SpotApi.ExchangeData`|
...
|WebSocket Asset & Order|✓|`socketClient.SpotApi`|


### Futures

*Futures is not currently supported*
...
## Release notes
* Version 1.2.0 - 24 Sep 2026
    * Updated CryptoExchange.Net to v13.0.0
    * Shared APIs
      * Added SharedApi V2 support
      * Added `.SharedApi` property to clients
      * Added `(I)LBankSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `LBankOptions` to provide SharedApi related options
    ...
    * Added additional OrderSide Enum value mappings

* Version 1.1.1 - 01 Sep 2026
    * Updated CryptoExchange.Net to v12.5.1
    * Fixed websocket concurrent subscription query update handling
